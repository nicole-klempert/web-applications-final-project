...
export const getFriendsByCity=async(req,res,next)=>{try{
    const user=await User.findById(req.session.user.id);
    if(!user)return res.status(404).json({success:false,error:'User not found'});
    const friendHashes=(user.friends||[]).map(username=>generateBlindIndex(username.toLowerCase().trim()));
    if(!friendHashes.length)return res.json({success:true,data:[],totalFriends:0});
    const data=await User.aggregate(cityPipeline({usernameHash:{$in:friendHashes}}));
